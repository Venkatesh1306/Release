...
uint16_t readInputStatus(uint8_t *p_modbusTxBuf, uint16_t *p_dataMemory, mbPacketParse_t *p_parseModbusTcpData)
{
  ...
  unsigned int NoOfBits = 0, Regbit = 0, reg = 0, RegbitTx = 0, regTx = 0, byte_index = 0, bit_count = 0, wordOffset = 0, n = 0;
  uint16_t Txtempbuf1[MaxSizeTxtempbuf];

  for (n = 0; n <= MaxSizeTxtempbuf; n++)
  {
    Txtempbuf1[n] = 0; /* initializing all values of Txtempbuf[m] = 0 */
  }
