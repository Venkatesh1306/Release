...
uint16_t Test_ing(const uint8_t mbTxBuf[], const uint8_t testTxBuf[])
{
    ...
    if (ModbusRxBuf[1] == 0x06)
    {
        add = (ModbusRxBuf[2] << 8) | ModbusRxBuf[3];
        test_c = (Dataregister[add] != ((ModbusRxBuf[4] << 8) | ModbusRxBuf[5])) ? 1 : 0;
    }

    // Data enter check for FC  0x10
    if (ModbusRxBuf[1] == 0x10)
    {
        add = (ModbusRxBuf[2] << 8) | ModbusRxBuf[3];
        numRegisters = (ModbusRxBuf[4] << 8) | ModbusRxBuf[5];

        for (increment = 0; increment < numRegisters; increment++)
        {
            receivedData = (ModbusRxBuf[7 + increment * 2] << 8) | ModbusRxBuf[8 + increment * 2];
            test_c = (Dataregister[add + increment] != receivedData) ? 1 : 0;
            // printf("%04x - %04x / ",receivedData,Dataregister[add+increment]);
        }
    }

    numRegisters = (mbTxBuf[1] < 0x80) ? (0x05 + mbTxBuf[2]) : 0x05;
    // send packet check for all function code
    for (increment = 0; increment < numRegisters; increment++)
    {
        // Test_Res = (mbTxBuf[increment] != testTxBuf[increment]) ? 0 : 1;
        if ((mbTxBuf[increment] != testTxBuf[increment]))
        {
            Test_Res = 0;
